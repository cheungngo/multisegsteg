#' Fit a multiple threshold segmented regression model
#'
#' @description
#' Fits a segmented regression model with multiple thresholds that allows for changes in
#' slope while maintaining continuity at the threshold points. The function employs a
#' grid search approach to identify optimal threshold values that maximize the model's
#' log-likelihood.
#'
#' @details
#' The segmented regression model implemented here follows the form:
#' \deqn{y = X\beta + \sum_{j=1}^{k} \gamma_j (z - e_j)_+ + \epsilon}
#' where \eqn{(z - e_j)_+ = \max(0, z - e_j)}, \eqn{e_j} are the threshold values,
#' \eqn{k} is the number of thresholds, and \eqn{X} represents covariates from formula.1.
#'
...
#'
#' # Fit model with one threshold
#' result1 <- multi_segmented(
#'   formula.1 = y ~ z,
#'   formula.2 = ~ x,
#'   family = "gaussian",
#'   data = data,
...
#' )
#'
#' # Fit model with two thresholds and cluster-robust standard errors
#' result2 <- multi_segmented(
#'   formula.1 = y ~ z,
#'   formula.2 = ~ x,
#'   family = "gaussian",
#'   data = data,
...
#' print(result2$se_cluster)
#'
#' @seealso \code{\link{plot_multi_segmented}} for visualizing the results
#' @seealso \code{\link{multi_stegmented}} for fitting models with both discontinuities and slope changes
#'
#' @importFrom sandwich vcovCL
#'
#' @export
multi_segmented <- function(formula.1, formula.2, family, data, n_thresholds = 1,
  ...
  for (i in 1:n_comb) {
    ...
    # Create threshold terms (x - e_i)+ for each threshold
    X_threshold <- sapply(thresholds, function(e) pmax(chngpt_var - e, 0))

    # Combine base covariates with threshold terms
...
#' Plot the results of a multi-segmented regression model
#'
#' @description
...
#'
#' 1. **Fit Plot**: Shows the original data points and the fitted segmented regression line,
...
#'
#' @param result List. Output from the multi_segmented function.
#' @param data Data frame. The original dataset used to fit the model.
#' @param formula.1 Formula. The same formula.1 used in the multi_segmented function.
#' @param formula.2 Formula. The same formula.2 used in the multi_segmented function.
...
#' print(loglik_plot)
#'
#' @seealso \code{\link{multi_segmented}} for fitting the segmented regression model
#' @seealso \code{\link{plot_multi_stegmented}} for plotting step-segmented regression models
#'
...
#'
#' @export
plot_multi_segmented <- function(result, data, formula.1, formula.2, plot.type = "both",
                                         n_points = 100, ...) {
  ...
  p1 <- ggplot(plot_data, aes(x = x, y = y)) +
    ...
    labs(title = paste(n_thresholds, "Threshold(s) Segmented Model Fit"),
         x = chngpt_var_name, y = "Response") +
    theme_minimal()

  # Plot 2: Log-likelihood Visualization
  if (n_thresholds == 2) {
    # For two thresholds, create a heatmap of log-likelihoods
    ...
    # For one threshold, plot log-likelihood vs threshold
    loglik_data <- data.frame(
    ...
    # For more than two thresholds, simplify to text output
    p2 <- ggplot() +
